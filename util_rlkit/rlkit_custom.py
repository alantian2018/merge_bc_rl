...
def rollout(
    ...
    #    print(path_length, max_path_length)
        ...
        if video_writer is not None:
            ...
            img = np.asarray(full_obs[env.camera_names[0] + "_image"])

            # Write to video writer
            img  = img[::-1]
            
            img = color_keypoints(env.pixels, img)

            text = f'Reward: {round(r,3)}'
            ...
            x = 512 - text_width - 10  # 10 pixels from the right edge
            y = text_height + 10  # 10 pixels from the top edge

            # Add the text to the image
            img = cv2.putText(np.ascontiguousarray(img, dtype=np.uint8), text, (x, y), font, font_scale, color, thickness)
